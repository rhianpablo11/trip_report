...
function enviarQuantsDaLista(ida:number, volta:number, vespertino:number){
...
function enviarListaIda(){
    let conteudo = window.encodeURIComponent(editaListaIda());
...
function enviarListaMatutino(){
    let conteudo = window.encodeURIComponent(editaListaMatutino());
    let url = "https://api.whatsapp.com/send?text="+conteudo;
    var win = window.open(url, '_blank');
    win.focus();
}
...
function enviarListaVespertino(){
    let conteudo = window.encodeURIComponent(editaListaVespertino());
    let url = "https://api.whatsapp.com/send?text="+conteudo;
    var win = window.open(url, '_blank');
    win.focus();
}
